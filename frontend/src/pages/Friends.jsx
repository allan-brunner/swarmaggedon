...
const Friends = () => {
  ...
  const [searchFriends, { loading: searchLoading, data: searchData }] =
      ...
      <>
        ...
        <div id="section-account" className="section-content active">
          <div style={{ textAlign: "center", padding: "100px" }}>
            <p>
              {authLoading ? t("friends.loading") : t("friends.redirecting")}
            </p>
          </div>
        </div>
      </>
  ...
  const pending_incoming_requests =
    friendsData?.pending_incoming_requests || [];
  const pending_outgoing_requests =
    friendsData?.pending_outgoing_requests || [];
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const { data } = await searchFriends({
        variables: { usernameSearch: searchUsername },
      });
  ...
  const handleAddFriend = async (userId, fromSearch = true) => {
    try {
      const { data } = await addFriendMutation({ variables: { userId } });
            ...
            <div className="panel-body">
              <div className="scroll-y">
                {loadingFriends ? (
                  ...
                  [...friends]
                    .sort((a, b) => {
                      const statusA = a.in_game
                        ? 2
                        : isUserOnline(a.last_online)
                          ? 1
                          : 0;
                      const statusB = b.in_game
                        ? 2
                        : isUserOnline(b.last_online)
                          ? 1
                          : 0;
                          ...
                          <div className="friend-info">
                            ...
                            <div className="friend-label">
                              ...
                              <div className="inline-status">
                                <div
                                  className={`dot ${friend.in_game ? "in-game" : isUserOnline(friend.last_online) ? "dot-online" : "dot-offline"}`}
                                ></div>
                                <div
                                  style={{
                                    fontSize: "14px",
